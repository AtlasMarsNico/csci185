...
function search() {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].Department === searchTerm) {
            ...
            const snippet = `<section class="course">
                <h2>${data[i].Code} ${data[i].Title}</h2>
                <p>${data[i].Days} ${data[i].Location.FullLocation} ${data[i].Hours} credit hour(s)</p>
                <p><strong>${data[i].Instructors[0].Name}</strong></p> </section>`;
